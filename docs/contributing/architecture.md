...
| `pkg/typescript` | TypeScript package consuming the WASM output |
...
The SDK uses a retryable HTTP client with the following policy:
```go
...
| Never retried | `POST` requests to prevent duplicate resource creation |
| Max attempts | 3 |
...
```ts
// Work In Progress. This section will be completed once the WASM implementation is finalized.

// When completed, this section should document:
// - How Go functions are exported and made callable from JavaScript via `syscall/js`
